    ...
    <div class="row d-flex align-items-center px-5" style="height: 85vh;">
        ...
        <div class="col-md-6">
            <div class="card border-0 p-5">
                <form action="/admin/update_kontak/<?= $kontak['id'] ?>" method="post" class="edit-form">
                    <h2 class="text-center judul mb-4">Edit Kontak</h2>
                    <div class="mb-3">
                        <input type="email" class="form-control input-kontak shadow-none" id="floatingInput" placeholder="Masukan email anda disini..." name="email" required value="<?= $kontak['email'] ?>"/>
                    </div>
                    <div class="mb-3">
                        <input type="text" class="form-control input-kontak shadow-none" id="floatingInput" placeholder="Pertanyaan Anda.." name="masalah" required value="<?= $kontak['masalah'] ?>"/>
                    </div>
                    <button type="submit" class="button-kontak border-0 fs-4 fw-bold">Simpan</button>
                </form>
            </div>
        </div>
    </div>
